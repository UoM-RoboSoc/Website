...
import CommitteeCard from '@site/src/components/Committee/CommitteeCard';
...
const ProjectList = [
  ...
  },
  {
    ...
    description: (
      <>
        R.F.L (Robot Fighting League) is a mechanical and electrical-focused project where members will be supported and guided in making fighting robots of different weight classes from ant weight (150 g) to hobby weight (5.44 kg).
  ...
  },
  {
    ...
    description: (
      <>
        C.U.B.O (Cube Utilising Brutal Over-engineering) works on building autonomous robots capable of solving a Rubik's cube in around 60-70 seconds!
...
function RegisterButton() {
  return (
    <Link className="button button--primary button--lg" to="#register">Register</Link>
  );
}

function Project({projectLead, img, title, difficulty, description, contact}) {
  return (
    <div style={{ '--bg': `url(${useBaseUrl(img)})`}} className={clsx('hero', styles.projectContainer)}>
      <div className={clsx("container")}>
        <div className="row">
          ...
          <div className="col col--3">
            ...
            <RegisterButton/>
            <br/><br/>
            <CommitteeCard name={projectLead} role={`${title} Lead`}/>
          </div>
        </div>
      </div>
    </div>
